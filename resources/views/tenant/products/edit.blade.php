...
@section('page-title', 'Update SKU Details')
...
<div class="max-w-4xl mx-auto">
    <div class="glass-card rounded-[2rem] border border-slate-100 dark:border-slate-800/80 overflow-hidden shadow-2xl shadow-blue-100 dark:shadow-none bg-white dark:bg-slate-900/40">
        <div class="p-8 bg-gradient-to-r from-blue-600 to-violet-700 text-white flex justify-between items-center">
            <div>
                <h3 class="text-xl font-black tracking-tight">Edit Product</h3>
                <p class="text-blue-100 text-[10px] font-bold uppercase tracking-widest">Update master entry for {{ $product->product_name }}</p>
            </div>
        ...
        <form action="{{ route('tenant.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6 bg-white/30 dark:bg-transparent">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" name="mrp" required value="{{ old('mrp', $product->mrp) }}"
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                <span class="absolute left-4 top-3.5 text-slate-400 dark:text-slate-500 font-bold text-xs">₹</span>
                                <input type="number" step="0.01" name="dealer_price" value="{{ old('dealer_price', $product->dealer_price) }}"
                                       class="w-full pl-8 pr-4 py-3 bg-white dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-black text-slate-900 dark:text-white focus:ring-4 focus:ring-emerald-50 dark:focus:ring-slate-800 outline-none transition-all">
                            </div>
                        </div>
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" min="0" max="100" name="discount_percent" value="{{ old('discount_percent', $product->discount_percent ?? 0) }}" placeholder="0 for no discount"
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="gst" class="w-full px-4 py-3 bg-white dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-bold text-slate-800 dark:text-slate-200 focus:ring-4 focus:ring-blue-50 dark:focus:ring-slate-800 outline-none transition-all appearance-none">
                                @foreach([0, 5, 12, 18, 28] as $rate)
                                    <option value="{{ $rate }}" {{ old('gst', $product->gst) == $rate ? 'selected' : '' }}>{{ $rate }}%</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="space-y-1">
                    ...
                    <div class="flex items-center gap-6 pt-2">
                        <div class="flex items-center gap-2">
                            ...
                            <input type="checkbox" name="is_active" value="1" {{ $product->is_active ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-slate-350 dark:border-slate-750 rounded focus:ring-blue-500 bg-white dark:bg-slate-950">
                            <label class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Active Product</label>
                        </div>
                        <div class="flex items-center gap-2">
                            <input type="hidden" name="pre_order_available" value="0">
                            <input type="checkbox" name="pre_order_available" value="1" {{ $product->pre_order_available ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-slate-350 dark:border-slate-750 rounded focus:ring-blue-500 bg-white dark:bg-slate-950">
                            <label class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Pre-Order Available</label>
                        </div>
                    </div>

                    <div class="flex items-center gap-2 p-3 bg-indigo-50/50 dark:bg-indigo-950/20 rounded-xl border border-indigo-100 dark:border-indigo-900/30">
                        <input type="hidden" name="show_on_website" value="0">
                        <input type="checkbox" name="show_on_website" id="show_on_website" value="1" {{ old('show_on_website', $product->show_on_website) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-350 dark:border-slate-750 rounded focus:ring-indigo-500 bg-white dark:bg-slate-950">
            ...
            <div class="pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    @if($product->image)
                        <img src="{{ tenant_asset($product->image) }}" class="w-10 h-10 rounded-xl object-cover border border-slate-200 dark:border-slate-800">
                    @endif
                     <label class="flex items-center gap-2 cursor-pointer group">
                        <input type="file" name="image" class="hidden" onchange="document.getElementById('img-label').innerText = 'New Image Selected'">
                        ...
                        <span id="img-label" class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Change Image</span>
                    </label>
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" class="px-10 py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-200 dark:shadow-none hover:scale-[1.05] active:scale-95 transition-all">Update Product</button>
                </div>
            </div>
        </form>
    </div>
