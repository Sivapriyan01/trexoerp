...
<div class="flex h-[calc(100vh-3.5rem)] -mx-4 md:-mx-6 -my-4 md:-my-6 overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto h-full p-6 md:p-8 space-y-8 custom-scrollbar bg-slate-50/20 dark:bg-slate-950/5">
        ...
        <div class="page-header">
            <div>
                <h1><i class="ti ti-report-money"></i> Expense Report</h1>
                <div class="breadcrumb">
                    <a href="{{ route('tenant.businessreport.index') }}">Reports</a> › Expenses
                </div>
            </div>
            <div class="flex gap-3">
                ...
                <button onclick="exportTableToCSV('expense-table', 'expense_report.csv')" class="btn btn-emerald">
                    <i class="ti ti-download"></i> Export
                </button>
            </div>
        </div>

        <form action="{{ route('tenant.businessreport.expense') }}" method="GET" class="filter-bar">
            <div>
                <label>Year</label><br>
        ...
        <div class="card">
            <div class="card-title"><i class="ti ti-list"></i> Expense Details</div>
            <div class="overflow-x-auto">
                <table id="expense-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-xs font-bold text-slate-500">{{ $expense->expense_date->format('d M Y') }}</td>
                            <td><span class="badge bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-1 rounded text-xs font-bold">{{ $expense->category }}</span></td>
                            <td class="text-sm text-slate-600 dark:text-slate-400">{{ $expense->description }}</td>
                            <td><span class="text-xs font-bold text-slate-500 uppercase">{{ $expense->payment_mode }}</span></td>
                            <td class="font-bold text-rose-600">₹{{ number_format($expense->amount, 2) }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="5" class="text-center py-12 text-slate-400 italic">No expenses found for this period.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
