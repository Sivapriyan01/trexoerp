...
<div class="flex h-[calc(100vh-3.5rem)] -mx-4 md:-mx-6 -my-4 md:-my-6 overflow-hidden">
    @include('tenant.partials.reports_sidebar', ['active' => 'payment'])
    ...
    <div class="flex-1 overflow-y-auto h-full p-6 md:p-8 space-y-8 custom-scrollbar bg-slate-50/20 dark:bg-slate-950/5">
        ...
        <div class="page-header">
            <div>
                <h1 style="margin: 0; display: flex; align-items: center; gap: 6px;"><i class="ti ti-credit-card" style="color:#4f7cff;"></i> Payment Report</h1>
                <div class="breadcrumb" style="margin-top: 2px;">
                    <a href="{{ route('tenant.businessreport.index') }}">Reports</a> › Payment Report
                </div>
            </div>
            <div class="btn-group">
                ...
                <button onclick="exportTableToCSV('payments-history-table', 'payment_report_{{ request('month', 'all') }}_{{ $year }}.csv')" class="btn btn-primary">
        ...
        {{-- Filter Bar --}}
        <form action="{{ route('tenant.businessreport.payment') }}" method="GET" class="filter-bar">
            <div>
                <label>Year</label><br>
                <select name="year">
            ...
            <div>
                ...
                <select name="mode">
                    ...
                    <option value="upi" {{ request('mode') == 'upi' ? 'selected' : '' }}>UPI / Online</option>
                    <option value="credit" {{ request('mode') == 'credit' ? 'selected' : '' }}>Credit / Due</option>
                </select>
            </div>
        ...
        <div class="card">
            <div class="card-title">
                <i class="ti ti-chart-bar"></i> Monthly Collections — {{ $year }}
            </div>
        ...
        {{-- Two Col: Payment Modes Distribution + Monthly Collection Table --}}
        <div class="two-col">

            {{-- Mode Breakdown --}}
            <div class="card">
                <div class="card-title">
                    <i class="ti ti-wallet"></i> Mode Breakdown
                </div>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>₹{{ number_format($mode['amount'], 2) }}</td>
                            <td>{{ $mode['count'] }}</td>
                            <td>
                                <div class="progress-wrap">
                                    <div class="progress-bar" style="width:{{ $mode['percent'] }}%"></div>
                                </div>
                            </td>
                        </tr>
                        @endforeach
            ...
            <div class="card">
                <div class="card-title">
                    <i class="ti ti-calendar"></i> Monthly Summary
                </div>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @php
                            ...
                            $collections = $chartData['collections'];
                        @endphp
                        @foreach ($labels as $i => $monthName)
                        <tr>
                            <td style="font-weight: 600;">{{ $monthName }}</td>
                            <td>{{ number_format($collections[$i]) }}</td>
                            <td>
                                @if ($i > 0)
                                    @php $diff = $collections[$i] - $collections[$i-1]; @endphp
        ...
        <div class="card" style="margin-top: 16px;">
            <div class="card-title">
                <i class="ti ti-list-details"></i> Transaction Ledger
            </div>
                ...
                <table id="payments-history-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $tx->bill_date->format('d M Y | h:i A') }}</td>
                            <td>
                                <div style="font-weight: 600;">{{ $tx->customer_name ?: ($tx->customer?->name ?: 'Walk-in') }}</div>
                                <div style="font-size: 11px; color: #888;">{{ $tx->customer_phone ?: 'N/A' }}</div>
                            </td>
                            <td>
                                @php
                            ...
                            <td>
                                <span style="display: inline-block; padding: 2px 8px; border-radius: 6px; font-size: 10px; font-weight: 700; text-transform: uppercase; {{ $tx->status === 'completed' ? 'background:#d1fae5;color:#065f46;' : 'background:#fee2e2;color:#991b1b;' }}">
                                    {{ $tx->status }}
                                </span>
                            </td>
